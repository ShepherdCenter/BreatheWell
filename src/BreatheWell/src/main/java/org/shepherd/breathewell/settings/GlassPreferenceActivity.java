package org.shepherd.breathewell.settings;

import java.util.ArrayList;
import java.util.List;
import org.shepherd.breathewell.App;
...
import org.shepherd.breathewell.settings.adapter.CardPreferenceAdapter;
...
import org.shepherd.breathewell.settings.types.TogglePreference;
import org.shepherd.breathewell.settings.types.ActivityPreference;
...
public class GlassPreferenceActivity extends Activity implements OnItemClickListener {
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		for (int i = 0; i < mCurrentOptions.size(); i++) {
            PreferenceOption p =mCurrentOptions.get(i);
            MenuItem item =menu.add(Menu.NONE, i, i, p.getTitle());
            View v = mScrollView.getSelectedView();
            if (p.getImageResource() != -1) {

               // v.setBackgroundResource(p.getImageResource());
                //item.set
            }
            else
            {
    ...
    public void rebuildCard(int index, AbstractPreference pref) {
        mAdapter.buildCard(index, pref);
        mAdapter.notifyDataSetChanged();
    }
    ...
    protected void addChoicePreference(String key, String title, int imageResource, List<PreferenceOption> options, int defaultValueIndex) {
        mAdapter.addPreference(new ChooserPreference(mPrefs, key, title, imageResource, options));
    }
	/**
	 * Add a Preference that gives a list of options. The index of the chosen option is saved.
	 */
