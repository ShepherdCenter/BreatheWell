...
public class ChooserPreference extends AbstractPreference{
	...
	public View getCard(Context context) {
		...
		View v = new CardBuilder(context, CardBuilder.Layout.MENU).setText(mBaseTitle).setFootnote(selected).getView();
        if (mOptions.get(selection).getImageResource() != -1) {
            v.setBackgroundResource(mOptions.get(selection).getImageResource());
        } else {
            if (mOptions.get(selection).getBitmap() != null) {
                BitmapDrawable bd = new BitmapDrawable(mOptions.get(selection).getBitmap());
                v.setBackground(bd);
            }
        }
        return v;
    ...
    public String getValue() {
        ...
        String selected  =  mOptions.get(selection).getValue();
        if (selected == null) {
            selected = mOptions.get(selection).getTitle();
        }
        return selected;
    }
	/**
